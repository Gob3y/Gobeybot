...
local function hatchLoop()
    ...
    task.spawn(function()
        while S.autoEgg and run == RunIds.autoEgg do
            local amount = math.clamp(math.floor(tonumber(S.eggAmount) or 1), 1, 25)
            local did = false
            for _ = 1, amount do
...
local function eventChaosLoop()
    ...
    task.spawn(function()
        local lastEvent = nil
        while S.autoEventChaos and run == S.run do
            local id = Workspace:GetAttribute("EventId")
...
local guiWidth = math.clamp(math.floor(viewport.X * (portrait and 0.78 or 0.38)), 220, 270)
local guiHeight = math.clamp(math.floor(viewport.Y * (portrait and 0.58 or 0.62)), 300, 390)
