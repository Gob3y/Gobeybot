...
-- Human-oriented timing (heuristic, not a universal standard).
-- The interval represents the time between answer inputs, with an occasional
-- longer thinking pause. Real players vary widely by puzzle and skill.
local solveModes = {
    NOOB = {label = "NOOB", min = 1.20, max = 2.60, pauseChance = 0.22, pauseMin = 1.20, pauseMax = 2.80},
    NORMAL = {label = "NORMAL", min = 0.75, max = 1.70, pauseChance = 0.16, pauseMin = 0.80, pauseMax = 2.00},
    PRO = {label = "PRO", min = 0.45, max = 1.10, pauseChance = 0.10, pauseMin = 0.60, pauseMax = 1.60},
}
...
local function serverFill(board, solution, runId)
    ...
    for _, entry in ipairs(pending) do
        ...
        if number then
            ...
            -- Human-oriented interval: random inside the selected skill range.
            -- A player normally needs a little time to read the next move, so
            -- this is intentionally slower than the old 0.12-0.32s PRO range.
            local interval = timing.min + math.random() * (timing.max - timing.min)
            if not cancellableWait(interval, runId) then
                return false, count, "Auto Solve dihentikan."
            end

            -- Some moves take longer to recognize. Keep this occasional rather
            -- than inserting a long pause after every cell.
            if math.random() < (timing.pauseChance or 0) then
                local pMin = timing.pauseMin or timing.min
                local pMax = timing.pauseMax or math.max(pMin, timing.min * 2)
                local thinkingPause = pMin + math.random() * math.max(0, pMax - pMin)
                if not cancellableWait(thinkingPause, runId) then
                    return false, count, "Auto Solve dihentikan."
                end
            end
        end
