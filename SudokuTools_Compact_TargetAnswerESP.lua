...
local function addTargetAnswerESP(cell, number)
    ...
    esp.Name = "SudokuTargetAnswerESP"
    esp.AlwaysOnTop = true
    esp.Size = UDim2.new(0, 42, 0, 22)
    ...
    table.insert(targetESPObjects, esp)
end
...
local function updateTargetText()
    ...
    if p then
        targetButton.Text = "  TARGET: " .. p.DisplayName .. "  •  @" .. p.Name
    else
...
local function rebuildTargetList()
    ...
    for _, entry in ipairs(boards) do
        ...
        if entry.userId ~= LocalPlayer.UserId then
            ...
            item.Text = "  " .. entry.display .. "  •  @" .. entry.name
...
targetESPButton = makeButton("○  TARGET ANSWERS  •  OFF", 6)
...
teleportButton = makeButton("➤  TELEPORT TO TARGET", 7)
...
print("Sudoku Tools - Compact Target Answer ESP loaded")
